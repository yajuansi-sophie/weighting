...
X<-data.frame(age=SRBIdata$age_dc,eth=SRBIdata$race_dc,edu=SRBIdata$edu)
#eldx childx wax +1 
          ...
          #for (i6 in unique(dat$eldx)){
            #for (i7 in unique(dat$childx)){
              #for (i8 in unique(dat$wax)){
                j<- (i1-1) * J_eth * J_edu + (i3-1) * J_edu + i4
                cell_id[dat$age==i1& dat$eth==i3 & dat$edu==i4]<-j
                cell_str[j,]<-c(i1,i3,i4)
              }
     }
}
...
#simulate Y
beta<-rnorm(8,seq(5,1,length.out=8),1)
Xdata<-matrix(0,n,8)
Xdata[,1]<-rep(1,n)
for (j in 1:7){
Xdata[,j]<-dat[,j]
...
summary(lm(Y ~ age + eth + edu + age:eth:edu, data=dat))
...
set_cppo("debug")
S.compile <- stan(file='stan/mrpweights021315b.stan',
                  data=stan.data,
                  iter=2, chains=1)

S <- stan(fit=S.compile,
          data=stan.data,
          iter=500, chains=1, seed=20150303)
print(S,digits=3)
...
S_cell <- stan(fit=S.compile_cell,
          data=stan.data_cell,
          iter=1000, chains=3, seed=20150303)
...
alphas <- as.data.frame(array(NA, c(n_sex*J_eth*n_inc*n_mar*J_edu*J_age*n_pid*n_prmd*n_votes*n_stt, 176)))
...
cat("sex_eth\n"); alphas$sex_eth <- (alphas$sex - 1) * J_eth + alphas$eth
cat("sex_inc\n"); alphas$sex_inc <- (alphas$sex - 1) * n_inc + alphas$inc
cat("sex_mar\n"); alphas$sex_mar <- (alphas$sex - 1) * n_mar + alphas$mar
cat("sex_edu\n"); alphas$sex_edu <- (alphas$sex - 1) * J_edu + alphas$edu
cat("sex_age\n"); alphas$sex_age <- (alphas$sex - 1) * J_age + alphas$age
...
cat("eth_edu\n"); alphas$eth_edu <- (alphas$eth - 1) * J_edu + alphas$edu
cat("eth_age\n"); alphas$eth_age <- (alphas$eth - 1) * J_age + alphas$age
...
cat("inc_edu\n"); alphas$inc_edu <- (alphas$inc - 1) * J_edu + alphas$edu
cat("inc_age\n"); alphas$inc_age <- (alphas$inc - 1) * J_age + alphas$age
...
cat("mar_edu\n"); alphas$mar_edu <- (alphas$mar - 1) * J_edu + alphas$edu
cat("mar_age\n"); alphas$mar_age <- (alphas$mar - 1) * J_age + alphas$age
...
cat("edu_age\n"); alphas$edu_age <- (alphas$edu - 1) * J_age + alphas$age
...
betas <- as.data.frame(expand.grid(1:stan.data$J_eth,
                                   1:stan.data$n_pid,
                                   1:stan.data$n_prmd))
