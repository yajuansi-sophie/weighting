...
#do not use the ACS weights
acs_uw_ad <-svydesign(id=~1, data=acs_ad)

acs_N<-as.numeric(svytable(~age_dc+race_dc+educat,acs_ds_ad))
acs_uw_N<-as.numeric(svytable(~age_dc+race_dc+educat,acs_uw_ad))

summary(acs_N/acs_uw_N)
sd(acs_N/acs_uw_N)
...
lambda_m <- runif(q,0,1)
  abs(rnorm(q,0,1))
lambda_inter <- runif(2,0,1)
  abs(rnorm(2,0,1))
