...
dat<-data.frame(Y=Y_sys,age=SRBIdata$age_dc,eth=SRBIdata$race_dc,edu=SRBIdata$educat,
                sex=SRBIdata$sex,inc=SRBIdata$opmres_x,eldx=SRBIdata$eldx_ca,
                cldx=SRBIdata$childx_ca,psx=SRBIdata$personx_ca)
            ...
            for (i2 in 1:J_eth){
              for (i1 in 1:J_age){
                ...
                pop_cell_id[acs_ad$age_dc==i1& acs_ad$race_dc==i2 & acs_ad$educat==i3&
                              ...
                              acs_ad$childx_ca==i7& acs_ad$personx_ca==i8]<-j
                cell_id[dat$age==i1 & dat$eth==i2 & dat$edu==i3 &
                          dat$sex==i4 & dat$inc==i5 & dat$eldx==i6&
                          dat$cldx==i7& dat$psx==i8]<-j
...
J_true<-length(N_cell_true) #8874
...
J_pop<-as.numeric(names(table(pop_cell_id))) #ACS occupiled cell

#N_cell_acs<-as.numeric(table(pop_cell_id))[as.numeric(names(table(pop_cell_id))) %in% as.numeric(names(table(cell_id)))]
...
###Assign Nj=0 when cell j is in the sample but not in the population###
### weight as 1???
#J_pop<-c(J_pop_acs,as.numeric(names(table(cell_id)))[!as.numeric(names(table(cell_id))) %in% as.numeric(names(table(pop_cell_id)))])

N_cell<-rep(0,J)
for (j in 1:J){
  if (J_use[j]%in%J_pop_acs){
    N_cell[j]<-as.numeric(table(pop_cell_id))[J_pop==J_use[j]]
  }
}
...
S.compile_cell_sp <- stan(file='stan/mrpweights05302016-8var-srbi.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)
...
dim(output$ps_w)
apply(output$ps_w,2,median)/apply(output$ps_w,2,mean)
apply(output$w_new,2,median)/apply(output$w_new,2,mean)
...
pop.ps<-data.frame(1:J_ps,Freq=as.numeric(table(acs_ad$personx_ca)))
...
dim(output_iid$ps_w)
apply(output_iid$ps_w,2,median)

hist(apply(output$ps_w,2,median)/apply(output_iid$ps_w,2,median))

apply(output_iid$ps_w,2,median)/apply(output_iid$ps_w,2,mean)
apply(output_iid$w_new,2,median)/apply(output_iid$w_new,2,mean)

length(unique(apply(output_iid$w_new,2,mean)))
plot(apply(output_iid$y_cell_new,2,mean)-y_cell) #compare with observed data


#age eth edu sex inc eldx cldx psx
age1 <- as.numeric(names(table(pop_cell_id))) %in% (1:J_sup)[cell_str[,1]==1]

sum(apply(output_iid$mu_cell_pred,2,mean)[age1] * N_cell_true[age1])/sum(N_cell_true[age1])

sm<-750
age1_sm<-rep(0,sm)
for (s in 1:sm){
age1_sm[s]<-sum(output_iid$mu_cell_pred[s,age1] * N_cell_true[age1])/sum(N_cell_true[age1])
}
summary(age1_sm)
quantile(age1_sm,c(0.025,0.5,0.975))
sd(age1_sm)

sex1 <- as.numeric(names(table(pop_cell_id))) %in% (1:J_sup)[cell_str[,5]==1]

sum(apply(output_iid$mu_cell_pred,2,mean)[sex1] * N_cell_true[sex1])/sum(N_cell_true[sex1])
...
save.image("output/srbi_8var_20160530.RData")
