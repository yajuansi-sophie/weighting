...
betaI_inc<-matrix(seq(-1, 0,length=dim(inc)[2]),dim(inc)[2],1)
...
#case 2)
betaI_age<-matrix(seq(0.5, 4,length=dim(age)[2]),dim(age)[2],1)
betaI_race<-matrix(rep(1,dim(race)[2]) + c(2,rep(0,dim(race)[2]-1)),dim(race)[2],1)
betaI_edu<-matrix(seq(3, 0,length=dim(edu)[2]),dim(edu)[2],1)
betaI_sex<-matrix(seq(1, 0,length=dim(sex)[2]),dim(sex)[2],1)
betaI_inc<-matrix(seq(-3, 0,length=dim(inc)[2]),dim(inc)[2],1)
betaI_cldx<-matrix(seq(0, 2,length=dim(cldx)[2]),dim(cldx)[2],1)
betaI_eldx<-matrix(seq(2, 0,length=dim(eldx)[2]),dim(eldx)[2],1)
betaI_psx<-matrix(seq(4, 0,length=dim(psx)[2]),dim(psx)[2],1)
...
stan.data_cell <- list(n=nrow(dat),q=q,
                       ...
                       J_true=J_true,J_use=J_use,J_sup=J_sup,
...
S.compile_cell_sp <- stan(file='stan/mrpweights05042016-5var.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)

S.compile_cell_1 <- stan(file='stan/mrpweights05242016-8var.stan',
                         data=stan.data_cell,
                         iter=2, chains=1)
...
#S.compile_cell_sp <- stan(file='stan/mrpweights03062016-3var.stan',
#                         data=stan.data_cell,
#                         iter=2, chains=1)
...
############################################################################################################
# load("output/check_3var_20160425.RData")
# 
# mean(bias_mu_pred)
# mean(bias_mu_pred^2)
# mean(sd_mu_pred)
