...
###Author: YS
###Latest Edit date: 05/23/2016
#setwd("/Users/Shared/ysi/GoogleDriveFolder/working/weighting/code")
#setwd("/Users/Shared/ysi/boxsync/Box Sync/weighting/code")
...
sel_prob <- 1/(1+exp(-(-2.5+age%*%betaI_age + race %*% betaI_race + edu%*%betaI_edu + 
...
S.compile_cell_sp <- stan(file='stan/mrpweights05052016-3var.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)

S.compile_cell_iid <- stan(file='stan/mrpweights05052016-3var-iid.stan',
                         data=stan.data_cell,
                         iter=2, chains=1)

S.compile_cell_sp_2 <- stan(file='stan/mrpweights05192016-3var.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)

S.compile_cell_iid_2 <- stan(file='stan/mrpweights05192016-3var-iid.stan',
                         data=stan.data_cell,
                         iter=2, chains=1)
...
S.compile_cell_sp_0 <- stan(file='stan/mrpweights05052016-3var.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)
...
st <- system.time(sflist_2 <- foreach(i.cores = 1:getDoParWorkers()) %dopar% {
  S2 <- stan(fit=S.compile_cell_sp_2,
            data=stan.data_cell,
            iter=500, chains=1, seed=stan.seed, chain_id=i.cores)
  return(S2)
})[3]
S2 <- sflist2stanfit(sflist_2)

#print(S,digits=3)
output<-extract(S, permuted=TRUE)
output2<-extract(S2, permuted=TRUE)
...
summary(output0$lambda_m[,1]*output0$sigma_m)
summary(output0$lambda_m[,2]*output0$sigma_m)
summary(output0$lambda_m[,3]*output0$sigma_m)

summary(output0$lambda_inter[,1]*output0$lambda_m[,1]*output0$lambda_m[,2]*output0$sigma_m)
summary(output0$lambda_inter[,1]*output0$lambda_m[,1]*output0$lambda_m[,3]*output0$sigma_m)
summary(output0$lambda_inter[,1]*output0$lambda_m[,2]*output0$lambda_m[,3]*output0$sigma_m)

summary(output0$lambda_inter[,2]*output0$lambda_m[,1]*output0$lambda_m[,2]*output0$lambda_m[,3]*output0$sigma_m)
...
plot(apply(output$y_cell_new,2,mean)-y_cell) 
...
# S.compile_cell_1 <- stan(file='stan/mrpweights04142016-3var-iid.stan',
#                          data=stan.data_cell,
#                          iter=2, chains=1)
...
dim(output_iid$ps_w)
apply(output_iid$ps_w,2,mean)-apply(output_iid$ps_w,2,median)
apply(output$ps_w,2,mean)-apply(output$ps_w,2,median)

summary(apply(output$ps_w,2,mean)/apply(output_iid$ps_w,2,mean))
...
w2<-apply(output_iid$w_new,2,mean)
w2_unit<-rep(0,n)
for (i in 1:n){
  w2_unit[i]<-apply(output_iid$w_new,2,mean)[J_use==cell_id[i]]
}
w2_unit<-w2_unit/mean(w2_unit)
summary(w2_unit)
sd(w2_unit)

sum(w2_unit*dat$Y)/sum(w2_unit)
...
save.image("output/check_3var_20160502_case6_Y2var_Imain.RData")

############################################################################################################
# load("output/check_3var_20160502_case1_all.RData")
# load("output/check_3var_20160502_case2_Ymain.RData")
load("output/check_3var_20160502_case3_Y2var.RData")
load("output/check_3var_20160502_case4_Yall_Imain.RData")
load("output/check_3var_20160502_case5_Ymain_Imain.RData")
load("output/check_3var_20160502_case6_Y2var_Imain.RData")

#cauchy prior
load("output/check_3var_20160505_case1_all.RData")
load("output/check_3var_20160505_case2_Ymain.RData")
load("output/check_3var_20160505_case3_Y2var.RData")
load("output/check_3var_20160505_case4_Yall_Imain.RData")
load("output/check_3var_20160505_case5_Ymain_Imain.RData")
load("output/check_3var_20160505_case6_Y2var_Imain.RData")

results<-rbind(c("ST",mean(bias_mu_pred),mean(bias_mu_pred^2),mean(sd_mu_pred),mean(cr_mu_pred)),
c("IND",mean(bias_mu_pred_iid),mean(bias_mu_pred_iid^2),mean(sd_mu_pred_iid),mean(cr_mu_pred_iid)),
c("ps",mean(bias_mu_ps),mean(bias_mu_ps^2),mean(sd_mu_ps),mean(cr_mu_ps)),
c("ips",mean(bias_mu_ips),mean(bias_mu_ips^2),mean(sd_mu_ips),mean(cr_mu_ips)),
c("Rake",mean(bias_mu_rake),mean(bias_mu_rake^2),mean(sd_mu_rake),mean(cr_mu_rake)))

print(xtable(results))

mean(bias_mu_sample)
mean(bias_mu_sample^2)
mean(sd_mu_sample)
mean(cr_mu_sample)

mean(bias_mu_sample_iid)
mean(bias_mu_sample_iid^2)
mean(sd_mu_sample_iid)
mean(cr_mu_sample_iid)

plot(apply(bias_mu_popcell^2,2,mean)/apply(bias_mu_popcell_iid^2,2,mean))
plot(apply(sd_mu_popcell,2,mean)/apply(sd_mu_popcell_iid,2,mean))
plot(apply(cr_mu_popcell,2,mean)/apply(cr_mu_popcell_iid,2,mean))

boxplot(c(apply(bias_mu_popcell,2,mean),apply(bias_mu_popcell_iid,2,mean))~c(rep("ST",100),rep("IND",100)))
boxplot(c(apply(bias_mu_popcell^2,2,mean),apply(bias_mu_popcell_iid^2,2,mean))~c(rep("ST",100),rep("IND",100)))
boxplot(c(apply(sd_mu_popcell,2,mean),apply(sd_mu_popcell_iid,2,mean))~c(rep("ST",100),rep("IND",100)))
boxplot(c(apply(cr_mu_popcell,2,mean),apply(cr_mu_popcell_iid,2,mean))~c(rep("ST",100),rep("IND",100)))


# apply(bias_mu_cell,2,mean)
# apply(bias_mu_cell^2,2,mean)
# apply(sd_mu_cell,2,mean)
# apply(cr_mu_cell,2,mean)
# 
# apply(bias_mu_cell_iid,2,mean)
# apply(bias_mu_cell_iid^2,2,mean)
# apply(sd_mu_cell_iid,2,mean)
# apply(cr_mu_cell_iid,2,mean)
