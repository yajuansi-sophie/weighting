...
# wax 0 1 2 acs_ad$wax_ca<-acs_ad$wax acs_ad$wax_ca[acs_ad$wax>1]<-2 personx 1-4
...
# case 1) betaY_age<-matrix(seq(0.5, 4,length=dim(age)[2]),dim(age)[2],1)
# betaY_race<-matrix(rep(1,dim(race)[2]) + c(2,rep(0,dim(race)[2]-1)),dim(race)[2],1) betaY_edu<-matrix(seq(3,
# 0,length=dim(edu)[2]),dim(edu)[2],1) betaY_sex<-matrix(seq(1, 0,length=dim(sex)[2]),dim(sex)[2],1)
# betaY_inc<-matrix(seq(-3, 0,length=dim(inc)[2]),dim(inc)[2],1) betaY_cldx<-matrix(0,dim(cldx)[2],1)
# betaY_eldx<-matrix(seq(2, 0,length=dim(eldx)[2]),dim(eldx)[2],1) betaY_psx<-matrix(seq(4,
# 0,length=dim(psx)[2]),dim(psx)[2],1)
...
betaY_race <- matrix(sample(-2:2, dim(race)[2], replace = T), dim(race)[2], 1)
...
betaY_cldx <- matrix(0, dim(cldx)[2], 1)
betaY_eldx <- matrix(0, dim(eldx)[2], 1)
...
muY <- age %*% betaY_age + race %*% betaY_race + edu %*% betaY_edu + sex %*% betaY_sex + inc %*% betaY_inc + psx %*% 
    betaY_psx + eldx %*% betaY_eldx + cldx %*% betaY_cldx + age_race %*% betaY_age_race + age_edu %*% betaY_age_edu + 
    race_edu %*% betaY_race_edu + race_inc %*% betaY_race_inc + age_inc %*% betaY_age_inc + inc_psx %*% betaY_inc_psx + 
    inc_eldx %*% betaY_inc_eldx + inc_cldx %*% betaY_inc_cldx + age_race_edu %*% betaY_age_race_edu + age_race_inc %*% 
    betaY_age_race_inc
...
# case 1) betaI_age<-matrix(seq(-1, 1,length=dim(age)[2]),dim(age)[2],1) betaI_race<-matrix(seq(-1,
# 1,length=dim(race)[2]),dim(race)[2],1) betaI_edu<-matrix(seq(-1, 1,length=dim(edu)[2]),dim(edu)[2],1)
# betaI_sex<-matrix(seq(-1, 0,length=dim(sex)[2]),dim(sex)[2],1) betaI_inc<-matrix(seq(0,
# 1,length=dim(inc)[2]),dim(inc)[2],1) betaI_cldx<-matrix(seq(0, 1,length=dim(cldx)[2]),dim(cldx)[2],1)
# betaI_eldx<-matrix(seq(0, 1,length=dim(eldx)[2]),dim(eldx)[2],1) betaI_psx<-matrix(seq(-1,
# 0,length=dim(psx)[2]),dim(psx)[2],1)
...
# sel_prob <- 1/(1+exp(-(-2+age%*%betaI_age + race %*% betaI_race + edu%*%betaI_edu + sex%*%betaI_sex + inc
# %*% betaI_inc + psx%*%betaI_psx + eldx%*%betaI_eldx + cldx %*% betaI_cldx + age_race%*%betaI_age_race +
# age_edu%*%betaI_age_edu + race_edu%*%betaI_race_edu + race_inc%*%betaI_race_inc + age_inc%*%betaI_age_inc +
# inc_psx%*%betaI_inc_psx + inc_eldx%*%betaI_inc_eldx + inc_cldx%*%betaI_inc_cldx +
# age_race_edu%*%betaI_age_race_edu + age_race_inc%*%betaI_age_race_inc)))
...
sel_prob <- 1/(1 + exp(-(-2 + age %*% betaI_age + race %*% betaI_race + edu %*% betaI_edu + sex %*% betaI_sex + 
    inc %*% betaI_inc + psx %*% betaI_psx + eldx %*% betaI_eldx + cldx %*% betaI_cldx + age_race %*% betaI_age_race + 
    age_edu %*% betaI_age_edu + race_edu %*% betaI_race_edu + race_inc %*% betaI_race_inc + age_inc %*% betaI_age_inc + 
    inc_psx %*% betaI_inc_psx + inc_eldx %*% betaI_inc_eldx + inc_cldx %*% betaI_inc_cldx + age_race_edu %*% betaI_age_race_edu + 
                    ...
                    for (i2 in 1:J_eth) {
                      for (i1 in 1:J_age) {
                        ...
                        pop_cell_id[acs_ad$age_dc == i1 & acs_ad$race_dc == i2 & acs_ad$educat == i3 & acs_ad$sex == 
                          i4 & acs_ad$opmres_x == i5 & acs_ad$eldx_ca == i6 & acs_ad$childx_ca == i7 & acs_ad$psx_ca == 
                          i8] <- j
...
dat <- data.frame(Y = Y[I], age = acs_ad$age_dc[I], eth = acs_ad$race_dc[I], edu = acs_ad$educat[I], sex = acs_ad$sex[I], 
    inc = acs_ad$opmres_x[I], eldx = acs_ad$eldx_ca[I], cldx = acs_ad$childx_ca[I], psx = acs_ad$psx_ca[I])
#-----------computation--------------#
### cell id
...
S.compile_cell_1 <- stan(file = "stan/mrpweights-8var-iid.stan", data = stan.data_cell, iter = 2, chains = 1)
...
cat("betaY_sex", betaY_sex, "\n")
cat("betaY_inc", betaY_inc, "\n")
cat("betaY_cldx", betaY_cldx, "\n")
cat("betaY_eldx", betaY_eldx, "\n")
cat("betaY_psx", betaY_psx, "\n")

cat("betaY_age_race", betaY_age_race, "\n")
cat("betaY_age_edu", betaY_age_edu, "\n")
cat("betaY_age_inc", betaY_age_inc, "\n")
cat("betaY_race_edu", betaY_race_edu, "\n")
cat("betaY_race_inc", betaY_race_inc, "\n")
cat("betaY_inc_psx", betaY_inc_psx, "\n")
cat("betaY_inc_cldx", betaY_inc_cldx, "\n")
cat("betaY_inc_eldx", betaY_inc_eldx, "\n")

cat("betaY_age_race_edu", betaY_age_race_edu, "\n")
cat("betaY_age_race_inc", betaY_age_race_inc, "\n")

cat("betaI_age", betaI_age, "\n")
cat("betaI_race", betaI_race, "\n")
cat("betaI_edu", betaI_edu, "\n")
cat("betaI_sex", betaI_sex, "\n")
cat("betaI_inc", betaI_inc, "\n")
cat("betaI_cldx", betaI_cldx, "\n")
cat("betaI_eldx", betaI_eldx, "\n")
cat("betaI_psx", betaI_psx, "\n")

cat("betaI_age_race", betaI_age_race, "\n")
cat("betaI_age_edu", betaI_age_edu, "\n")
cat("betaI_age_inc", betaI_age_inc, "\n")
cat("betaI_race_edu", betaI_race_edu, "\n")
cat("betaI_race_inc", betaI_race_inc, "\n")
cat("betaI_inc_psx", betaI_inc_psx, "\n")
cat("betaI_inc_cldx", betaI_inc_cldx, "\n")
cat("betaI_inc_eldx", betaI_inc_eldx, "\n")

cat("betaI_age_race_edu", betaI_age_race_edu, "\n")
cat("betaI_age_race_inc", betaI_age_race_inc, "\n")
...
dim(output_iid$ps_w)
hist(apply(output_iid$ps_w, 2, mean))
...
summary(output_iid$theta_pred)
quantile(output_iid$theta_pred, c(0.025, 0.975))
mean(Y)
sd(output_iid$theta_pred)


apply(output$mu_cell_pred, 2, mean) - mu_cell_true
sd_mu_popcell[r, ] <- apply(output$mu_cell_pred, 2, sd)

hist((apply(output$mu_cell_pred, 2, mean) - mu_cell_true)^2/((apply(output_iid$mu_cell_pred, 2, mean) - mu_cell_true)^2))
hist(apply(output$mu_cell_pred, 2, sd)/apply(output_iid$mu_cell_pred, 2, sd))
...
sqrt(sum((w_unit^2 - w_unit) * dat$Y^2))/n
sqrt(sum((w_unit^2 - w_unit) * var(dat$Y)))/n
...
sqrt(sum(w_unit_iid^2 * var(dat$Y)))/n
...
mean(w_ips * dat$Y) - mean(Y)
sqrt(sum(w_ips^2 * var(dat$Y)))/n
...
dat_rake <- rake(dat.design, list(~age, ~eth, ~edu, ~sex, ~inc, ~cldx, ~eldx, ~psx), list(pop.age, pop.eth, pop.edu, 
    pop.sex, pop.inc, pop.cld, pop.eld, pop.ps), control = list(maxit = 20, epsilon = 1, verbose = FALSE))
# Warning message: In rake(dat.design, list(~age, ~eth, ~edu, ~sex, ~inc, ~cldx, ~eldx, : Raking did not
# converge after 10 iterations.

w_rake <- weights(dat_rake)/mean(weights(dat_rake))


mean(w_rake * dat$Y) - mean(Y)
...
for (r in 1:R) {
    for (v in 1:q) {
        for (l in 1:l_v[v + 1]) {
            ...
            mar_true <- sum(mu_cell_true[J_pop %in% cell_index] * N_cell_true[J_pop %in% cell_index])/sum(N_cell_true[J_pop %in% 
                cell_index])
            # mean(Y[pop_data[,v]==l]) model prediction
            ...
            # independent prior
            for (s in 1:dim(output$mu_cell_pred)[1]) {
                iid_est_sm[s] <- sum(output_iid$mu_cell_pred[s, J_pop %in% cell_index] * N_cell_true[J_pop %in% 
                  cell_index])/sum(N_cell_true[J_pop %in% cell_index])
            }
...
for (r in 1:R) {
    ...
    cell_index <- (1:J_sup)[cell_str[, 1] == 1 & cell_str[, 2] == 2 & cell_str[, 3] > 2]  #73#use
    ...
    # independent prior
    for (s in 1:dim(output$mu_cell_pred)[1]) {
        iid_est_sm[s] <- sum(output_iid$mu_cell_pred[s, J_pop %in% cell_index] * N_cell_true[J_pop %in% cell_index])/sum(N_cell_true[J_pop %in% 
            cell_index])
    }
