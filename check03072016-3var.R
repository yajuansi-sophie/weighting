...
pdf("plot/fullps-wght03082016.pdf")
p <- ggplot() + geom_point(aes(x=n_cell,y=N_cell/n_cell/sum(N_cell)*n))
p + 
  scale_y_continuous("Fully poststratification cell weights") + 
...
dat<-cbind(dat,cell_id)
y_cell <- aggregate(.~cell_id,data=dat,mean)[,5]

ss_cell<-0
for (j in 1:J){
  ss_cell<-ss_cell+sum((dat$Y_sys[cell_id==J_use[j]]-y_cell[j])^2)
}
...
# p+scale_y_continuous(expression(paste("Variance of",theta, "(sum of variances)"))) + 
...
pdf("plot/model-wght03082016.pdf")
...
pdf("plot/model_wgh_prop03082016.pdf")
...
# 
# 
plot(apply(output$mu_cell,2,mean))
plot(apply(output$y_cell_new,2,mean))

plot(apply(output$mu_cell_pred,2,mean))
...
# 
pdf("plot/mu_bias03082016.pdf")
p <- ggplot() + geom_point(aes(x=n_cell,y=(apply(output$y_cell_new,2,mean)-y_cell)/y_cell))
p + 
  scale_y_continuous("Relative bias of cell mean estimation") + 
  scale_x_continuous(name="Sample cell sizes")+
  theme(
    legend.position="",
    legend.direction="horizontal",
