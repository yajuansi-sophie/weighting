...
for (j in 373:516){
  if (length(unique(SRBIdata[,j]))>4 & sum(SRBIdata[,j]=="NA")<1685){
    print(j)
    print(names(SRBIdata)[j])
    print(length(unique(SRBIdata[,j])))
    #print(table(SRBIdata[,j]))
  }
}
# bestlife=qd7
...
hist(SRBIdata[,399])
hist(SRBIdata$imp_r4_rentpaid)
plot(SRBIdata$opmres_x, SRBIdata$qa9x)
plot(SRBIdata$opmres_x, SRBIdata$imp_r5_earnhd)
plot(SRBIdata$opmres_x, SRBIdata$imp_r5_earnsp)
plot(SRBIdata$opmres_x, SRBIdata$imp_r5_incsnap)
plot(as.numeric(SRBIdata$povgap), SRBIdata$imp_r5_incsnap)
#dapply(SRBIdata$imp_r5_incsnap,summary,SRBIdata$povgap)
#plot(SRBIdata$opmrex_x, SRBIdata$imp_r5_incdis)
summary(SRBIdata$imp_r5_incsnap[SRBIdata$povgap=="1 Under 50%"])
summary(SRBIdata$imp_r5_incsnap[SRBIdata$povgap=="2 50-100%"])
summary(SRBIdata$imp_r5_incsnap[SRBIdata$povgap=="3 100-200%"])
summary(SRBIdata$imp_r5_incsnap[SRBIdata$povgap=="4 200-300%"])
summary(SRBIdata$imp_r5_incsnap[SRBIdata$povgap=="5 300%+"])
###ACS data
...
# acs_ad$age_dc<-age_tr #discretization
# acs_ad$age_dc[age_tr<=24]<-1 #18-34
# acs_ad$age_dc[age_tr<=34 & age_tr>24]<-2 #35-64
# acs_ad$age_dc[age_tr<=44 & age_tr>34]<-3
# acs_ad$age_dc[age_tr<=54 & age_tr>44]<-4
# acs_ad$age_dc[age_tr<=64 & age_tr>54]<-5
# acs_ad$age_dc[age_tr<=74 & age_tr>64]<-6
# acs_ad$age_dc[age_tr>74]<-7
# 
# SRBIdata$age_dc<-SRBIdata$age
# SRBIdata$age_dc[SRBIdata$age<=24]<-1
# SRBIdata$age_dc[SRBIdata$age<=34 & SRBIdata$age > 24]<-2
# SRBIdata$age_dc[SRBIdata$age<=44 & SRBIdata$age > 34]<-3
# SRBIdata$age_dc[SRBIdata$age<=54 & SRBIdata$age > 44]<-4
# SRBIdata$age_dc[SRBIdata$age<=64 & SRBIdata$age > 54]<-5
# SRBIdata$age_dc[SRBIdata$age<=74 & SRBIdata$age > 64]<-6
# SRBIdata$age_dc[SRBIdata$age > 74]<-7
...
#acs_N<-as.numeric(svytable(~age_dc+race_dc+educat+sex+opmres_x+eldx_ca+childx_ca+
#                             wax_ca,acs_ds_ad))
acs_N<-as.numeric(svytable(~age_dc+race_dc+educat+sex+opmres_x,acs_ds_ad))

#X<-data.frame(age=SRBIdata$age_dc,eth=SRBIdata$race_dc,edu=SRBIdata$educat,
#              sex=SRBIdata$sex,inc=SRBIdata$opmres_x,eld=SRBIdata$eldx_ca,
#              child=SRBIdata$childx_ca,wa=SRBIdata$wax_ca)
X<-data.frame(age=SRBIdata$age_dc,eth=SRBIdata$race_dc,edu=SRBIdata$educat,
              sex=SRBIdata$sex,inc=SRBIdata$opmres_x)
dat <- data.frame(X)
...
cell_id<-rep(0,n)
cell_str<-matrix(0,J_age*J_eth*J_edu*J_sex*J_inc,q)
j<-0
for (i5 in 1:J_inc){
  for (i4 in 1:J_sex){
    for (i3 in 1:J_edu){
      for (i2 in 1:J_eth){
        for (i1 in 1:J_age){
          j<- (i5-1) * J_sex * J_edu * J_eth * J_age + (i4-1) * J_edu * J_eth * J_age + 
            (i3-1) * J_eth * J_age + (i2-1) * J_age + i1
          cell_id[dat$age==i1& dat$eth==i2 & dat$edu==i3 & dat$sex==i4 & dat$inc==i5]<-j
          cell_str[j,]<-c(i1,i2,i3,i4,i5)
...
J_true<-J_age*J_eth*J_edu*J_sex*J_inc
...
#################################################
sigma_y <- abs(rnorm(1,0,1)*2.5)
sigma_m <- abs(rnorm(1,0,1)*2.5)
lambda_m <- abs(rnorm(q,0,0.5))
lambda_inter <- abs(rnorm(4,0,0.5))
...
alpha_eth_sex<-rnorm(J_sex * J_eth); alpha_edu_sex<-rnorm(J_sex * J_edu);
...
alpha_age_eth_sex <-rnorm(J_age * J_sex * J_eth);
alpha_age_edu_sex <-rnorm(J_age * J_sex * J_edu);
...
alpha_eth_edu_sex <-rnorm(J_sex * J_eth * J_edu);
alpha_eth_sex_inc <-rnorm(J_sex * J_eth * J_inc);
alpha_edu_sex_inc <-rnorm(J_sex * J_edu * J_inc);
alpha_eth_edu_inc <-rnorm(J_eth * J_edu * J_inc);  
alpha_age_eth_edu_sex <-rnorm(J_age * J_sex * J_eth * J_edu);
alpha_age_eth_sex_inc <-rnorm(J_age * J_sex * J_eth * J_inc);
alpha_age_edu_sex_inc <-rnorm(J_age * J_sex * J_edu * J_inc);
alpha_age_eth_edu_inc <-rnorm(J_age * J_eth * J_edu * J_inc);
alpha_eth_edu_sex_inc <-rnorm(J_sex * J_eth * J_edu * J_inc);  
alpha_age_eth_edu_sex_inc <-rnorm(J_age * J_sex * J_eth * J_edu * J_inc);  
...
S.compile_cell_2 <- stan(file='stan/mrpweights03022016-5var.stan',
                         data=stan.data_cell,
                         iter=2, chains=1)
...
st <- system.time(sflist1 <- foreach(i.cores = 1:getDoParWorkers()) %dopar% {
  S <- stan(fit=S.compile_cell_2,
            data=stan.data_cell,
            iter=300, chains=1, seed=stan.seed, chain_id=i.cores)
  return(S)
})[3]
