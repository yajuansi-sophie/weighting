...
for (r in 1:R){
  ...
  lambda_inter <-abs(rcauchy(4,0,1))

  pri_var <- sum(lambda_m^2 * sigma_m^2) + (lambda_inter[1] * sigma_m[1] * sigma_m[2])^2 +
    (lambda_inter[2] * sigma_m[1] * sigma_m[3])^2 + (lambda_inter[3] * sigma_m[2] * sigma_m[3])^2 +
    (lambda_inter[4] * sigma_m[1] * sigma_m[2]* sigma_m[3])^2
...
set_cppo("debug")
S.compile_cell <- stan(file='stan/mrp3var072015a.stan',
                       data=stan.data_cell,
                       iter=2, chains=1)
...
st <- system.time(sflist1 <- foreach(i.cores = 1:getDoParWorkers()) %dopar% {
  S <- stan(fit=S.compile_cell,
            data=stan.data_cell,
            iter=2000, chains=1, seed=stan.seed, chain_id=i.cores)
  return(S)
})[3]
