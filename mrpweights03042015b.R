...
#+eldx_ca+childx_ca+wax_ca
acs_N<-as.numeric(svytable(~age_dc+sex+race_dc+educat+opmres_x,acs_ds_ad))
###
    ...
    for (i3 in unique(dat$eth)){
      for (i2 in unique(dat$sex)){
        for (i1 in unique(dat$age)){
                j<- (i5-1) * J_edu* J_eth* J_sex * J_age + (i4-1) * J_eth* J_sex * J_age + (i3-1) * J_sex * J_age + (i2-1) * J_age + i1
                if (sum(dat$age==i1& dat$sex==i2 & dat$eth==i3 & dat$edu==i4 & dat$inc==i5)>0){
                cell_id[dat$age==i1& dat$sex==i2 & dat$eth==i3 & dat$edu==i4 & dat$inc==i5]<-j
...
N_cell<-acs_N
#X_n<-as.numeric(table(dat$age,dat$sex,dat$eth,dat$edu,dat$inc))
