...
max(N_cell/n_cell/sum(N_cell)*n)/min(N_cell/n_cell/sum(N_cell)*n)
plot(n_cell,N_cell/n_cell/sum(N_cell)*n)
hist(N_cell/n_cell/sum(N_cell)*n)
summary(N_cell/n_cell)
#-------------Raking weights------------------------#
pop.age <- data.frame(age=1:3, Freq=as.numeric(svytable(~age_dc,acs_ds_ad)))
pop.eth <- data.frame(eth=1:4, Freq=as.numeric(svytable(~race_dc,acs_ds_ad)))
pop.edu <- data.frame(edu=1:4, Freq=as.numeric(svytable(~educat,acs_ds_ad)))
dat_syn <- svydesign(id=~1, weights=~1, data=dat)
rake_wght <- weights(rake(dat_syn, list(~age,~eth,~edu), list(pop.age, pop.eth, pop.edu)))
#average to 1
rake_wght <- rake_wght/mean(rake_wght)
rake_cell <- rep(0, J)
for (j in 1:J){
  rake_cell[j]<-unique(rake_wght[cell_id==j])
}
summary(rake_cell)
sd(rake_cell)
#----------Coherence check--------------#
...
mean(cover_sigmay95)
hist(apply(cover_mu50,2,mean))
hist(apply(cover_mu80,2,mean))
hist(apply(cover_mu95,2,mean))

hist(apply(mu_bias,2,mean))
hist(apply(sd_bias,2,mean))
############################################################################################################
