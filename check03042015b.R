...
for (r in 1:R){
  sigma_y <- abs(rnorm(1))
  lambda_m <- abs(rnorm(1))
  sigma_m <- abs(rnorm(q))
  lambda_inter <-abs(rnorm(q-1))
...
set_cppo("debug")
S.compile_cell <- stan(file='stan/mrpweights031115b.stan',
                  data=stan.data_cell,
                  iter=2, chains=1)
...
save.image("check_mrp_20150311c.RData")

# load("check_mrp_20150306.RData")
