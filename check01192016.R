...
cell_id<-rep(0,n)
cell_str<-matrix(0,J_age*J_eth,q)
 j<-0
  for (i2 in 1:J_eth){
    for (i1 in 1:J_age){
                j<- (i2-1) * J_age + i1
                cell_id[dat$age==i1& dat$eth==i2]<-j
                cell_str[j,]<-c(i1,i2)
    }
  }
...
#################################################
  sigma_y <- abs(rnorm(1,0,2.5))
  sigma_m <- abs(rnorm(q+1,0,2.5))
  #lambda_inter <- abs(rnorm(1,0,1))

  pri_var <- sum(sigma_m^2)

  alpha<-3
  alpha_age <-rnorm(J_age); alpha_eth <-rnorm(J_eth);
  alpha_age_eth<-rnorm(J_age * J_eth); 

  mu_mean<-function(x_value){
    alpha + alpha_age[x_value[1]] * sigma_m[1] + 
      alpha_eth[x_value[2]] * sigma_m[2] + 
      alpha_age_eth[(x_value[2]-1) * J_age + x_value[1]] * sigma_m[3]
  } 
...
##################################################
#2) the same error variance, different scale
#################################################
...
pri_var <- sum(lambda_m^2 * sigma_m^2) + (lambda_inter * lambda_m[1] * lambda_m[2] * sigma_m)^2 
...
mu_mean<-function(x_value){
  alpha + alpha_age[x_value[1]] * lambda_m[1] * sigma_m + 
    ...
    alpha_age_eth[(x_value[2]-1) * J_age + x_value[1]] * lambda_inter * lambda_m[1] * lambda_m[2] * sigma_m
} 
...
stan.data_cell <- list(n=nrow(dat),q=q,
                       ...
                       #sigma_y=sigma_y, sigma_m=sigma_m
...
S.compile_cell_2 <- stan(file='stan/mrpweights12012015-2var-b.stan',
                         data=stan.data_cell,
                         iter=2, chains=1)
...
##################################################
#3) different scales and product of errors
#################################################
lambda_m <- 1
sigma_y <- abs(rnorm(1,0,1)*2.5)
sigma_m <- abs(rnorm(q,0,1)*2.5)
lambda_inter <- abs(rnorm(1,0,1))
...
mu_mean<-function(x_value){
  alpha + alpha_age[x_value[1]] * lambda_m * sigma_m[1] + 
    ...
    alpha_age_eth[(x_value[2]-1) * J_age + x_value[1]] * lambda_inter[1] * sigma_m[1] * sigma_m[2]
} 
...
S.compile_cell_1 <- stan(file='stan/mrpweights11172015-2var.stan',
                       data=stan.data_cell,
                       iter=2, chains=1)
S.compile_cell_1 <- stan(file='stan/mrpweights12012015-2var-a.stan',
                         data=stan.data_cell,
                         iter=2, chains=1)
