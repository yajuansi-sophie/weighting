...
###Author: YS
###Latest Edit date: 02/08/2017
#setwd("/Users/Shared/ysi/boxsync/Box Sync/projects/weighting/code")
#setwd("/Users/ysi/Box Sync/projects/weighting/code/")
...
S.compile_cell_sp2 <- stan(file='stan/mrpweights02072017-3var.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)
...
S.compile_cell_HS <- stan(file='stan/mrpweights02082017-3var-HS.stan',
                          data=stan.data_cell,
                          iter=2, chains=1)
...
n.chains <- 3
registerDoMC(n.chains)
st <- system.time(sflist2 <- foreach(i.cores = 1:getDoParWorkers()) %dopar% {
  S2 <- stan(fit=S.compile_cell_sp2,
            data=stan.data_cell,
            iter=500, chains=1, seed=stan.seed, chain_id=i.cores)
  return(S2)
})[3]
S2 <- sflist2stanfit(sflist2)
...
# length(unique(apply(output$ps_w,2,mean)))
...
save.image("output/check_3var_20160526_case6_Ymain_Imain.RData")

############################################################################################################
load("output/check_3var_20160526_case1_all.RData")
load("output/check_3var_20160526_case2_Yall_Imain.RData")
load("output/check_3var_20160526_case3_Ymain_Iall.RData")
load("output/check_3var_20160526_case4_Y2var_Iall.RData")
load("output/check_3var_20160526_case5_Y2var_Imain.RData")
load("output/check_3var_20160526_case6_Ymain_Imain.RData")

#load("output/check_3var_20160606_case1_all.RData")
...
weights<-cbind(c(w_unit,w_unit_iid,w_rake,w_ps,w_ips),c(rep("SPM",n),
                                                        rep("INPM",n),
               rep("Rake",n),rep("PS",n),rep("IPW",n)))
weights<-data.frame(weights)
names(weights)<-c("weights","Method")
weights$weights<-as.numeric(weights$weights)
weights$Method<-factor(weights$Method, levels=c("SPM","INPM","Rake","PS","IPW"))
sd(w_unit)

# # Interleaved histograms
# ggplot(weights, aes(x=weights, fill=Method)) +geom_density(alpha=.3)
#   geom_histogram(binwidth=5, position="identity") #dodge
pdf("/Users/Shared/ysi/boxsync/Box Sync/projects/weighting/latex/plot/weight-case1-07142016.pdf")
ggplot(weights, aes(x=weights)) + geom_histogram(binwidth=5, colour="black", fill="white") + 
  scale_x_continuous(name="Generated weights: Case 1")+
  facet_grid(Method ~ .)
dev.off()
#http://www.cookbook-r.com/Graphs/Plotting_distributions_(ggplot2)/

apply(wght_sd_rt,2,mean)
apply(wght_sd_rt_iid,2,mean)
apply(wght_sd_rt_ps,2,mean)

apply(wght_sd_rt_ips,2,mean)
apply(wght_sd_rt_rake,2,mean)

over_mean<-data.frame(rbind(c(mean(bias_mu_pred),
...
colnames(over_mean)<-c("bias", "RMSE", "Ave.SD", "CR")
rownames(over_mean)<-c("SPM", "INPM", "Rake", "PS", "IPW")

print(xtable(over_mean,digits=3))

boxplot(c(apply(bias_mu_popcell,2,mean),apply(bias_mu_popcell_iid,2,mean))~c(rep("ST",100),rep("IND",100)))
boxplot(c(sqrt(apply(bias_mu_popcell^2,2,mean)),sqrt(apply(bias_mu_popcell_iid^2,2,mean)))~c(rep("ST",100),rep("IND",100)))
...
apply(bias_mu_popcell^2,2,mean)
apply(sd_mu_popcell,2,mean)
apply(cr_mu_popcell,2,mean)

boxplot(apply(bias_mu_popcell_iid,2,mean))
apply(bias_mu_popcell_iid^2,2,mean)
apply(sd_mu_popcell_iid,2,mean)
apply(cr_mu_popcell_iid,2,mean)

hist(apply(bias_mu_popcell^2,2,mean)/apply(bias_mu_popcell_iid^2,2,mean))
hist(apply(sd_mu_popcell,2,mean)/apply(sd_mu_popcell_iid,2,mean))
