...
parameters {
  ...
  real<lower=0.9999, upper=1.0001> lambda_inter[2];
  real<lower=0.001> sigma_y[J_true];
...
for (j in 1:J){
  ...
  for (j in 1:J){
    sigma_y_cell[j] <- sigma_y[J_use[j]] / sqrt(n_cell[j]);
  }
...
generated quantities{
  ...
  for (j in 1:J){
    y_cell_new[j] <- normal_rng(mu_cell[j],sigma_y_cell);
