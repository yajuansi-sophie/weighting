...
parameters {
  ...
  real<lower=0> sigma_m[J_age_eth_edu];
...
for (j in 1:J){
  mu_cell[j] <- alpha + alpha_age[cell_str[J_use[j],1]] * sigma_m[1] + 
    ...
    alpha_eth_edu[(cell_str[J_use[j],3]-1) * J_eth + cell_str[J_use[j],2]] * sigma_m[6] + alpha_age_eth_edu[(cell_str[J_use[j],3]-1) * J_eth * J_age + (cell_str[J_use[j],2]-1) * J_age + cell_str[J_use[j],1]] * sigma_m[7];
}
