...
parameters {
  ...
  //real<lower=0.001> lambda_inter[2];
  //real<lower=0.001> sigma_y;
  real<lower=0.001> sigma_m[7];
  real<lower=0.001> sigma_y;
  real<lower=0.001> tau_cell[J];
...
for (j in 1:J){
  mu_cell[j] <- alpha + alpha_age[cell_str[J_use[j],1]] * sigma_m[1] + 
    ...
    alpha_age_eth[(cell_str[J_use[j],2]-1) * J_age + cell_str[J_use[j],1]] * sigma_m[4] +
    alpha_age_edu[(cell_str[J_use[j],3]-1) * J_age + cell_str[J_use[j],1]] * sigma_m[5] +
    alpha_eth_edu[(cell_str[J_use[j],3]-1) * J_eth + cell_str[J_use[j],2]] * sigma_m[6] + 
    alpha_age_eth_edu[(cell_str[J_use[j],3]-1) * J_eth * J_age + (cell_str[J_use[j],2]-1) * J_age + cell_str[J_use[j],1]] * sigma_m[7];
}
  for (j in 1:J){
    sigma_y_cell[j] <- sigma_y / sqrt(n_cell[j]);
    sigma_theta_cell[j] <- sigma_y / tau_cell[j];
  }
...
model {
  ...
  //lambda_m ~ cauchy(0,1);
  //tau_cell ~ cauchy(0,1);
// lambda_inter ~ double_exponential(0,1);
  //lambda_m ~ double_exponential(0,1);
  theta_cell ~ normal(mu_cell,sigma_theta_cell);
  y_cell ~ normal(theta_cell,sigma_y_cell);
...
generated quantities{
  ...
  pri_var <- pow(sigma_m[1],2)+pow(sigma_m[2],2)+pow(sigma_m[3],2)+pow(sigma_m[4],2)+pow(sigma_m[5],2)+
    pow(sigma_m[6],2)+pow(sigma_m[7],2);

  for (j in 1:J){
    y_cell_new[j] <- normal_rng(mu_cell[j],sqrt(pow(sigma_y_cell[j],2)+pow(sigma_theta_cell[j],2)));

    ps_w[j] <- 1/(pow(sigma_y_cell[j],2)+pow(sigma_theta_cell[j],2))/(1/(pow(sigma_y_cell[j],2)+pow(sigma_theta_cell[j],2)) + 1/pri_var);
 ...
 for (j in 1:J_true){
   mu_cell_pred[j] <- alpha + alpha_age[cell_str[j,1]] * sigma_m[1] + 
      alpha_eth[cell_str[j,2]] * sigma_m[2] + 
     alpha_edu[cell_str[j,3]] * sigma_m[3] +
      alpha_age_eth[(cell_str[j,2]-1) * J_age + cell_str[j,1]] * sigma_m[4] +
    alpha_age_edu[(cell_str[j,3]-1) * J_age + cell_str[j,1]] * sigma_m[5] +
    alpha_eth_edu[(cell_str[j,3]-1) * J_eth + cell_str[j,2]] * sigma_m[6] + 
    alpha_age_eth_edu[(cell_str[j,3]-1) * J_eth * J_age + (cell_str[j,2]-1) * J_age + cell_str[j,1]] * sigma_m[7];
  }
