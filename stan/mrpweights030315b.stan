data {
  ...
  vector[J] ss_cell; //sum of variance in cells
...
model {
  ...
  sum(ss_cell) / pow(sigma_y,2) ~ chi_square(n-1);
...
generated quantities{
  ...
  pri_var <- pow(lambda_m * sigma_m[1],2) + 
    pow(lambda_m * sigma_m[2],2) + 
    pow(lambda_m * sigma_m[3],2) + 
