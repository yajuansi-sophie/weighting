...
transformed parameters{
  ...
  //real<lower=0> sigma_m[q];
  //for (j0 in 1:q){
  //sigma_m[j0] <- sigma_m0[j0] * hyper_sg;
  //}
 ...
 for (j in 1:J){
    mu_cell[j] <- alpha + alpha_age[cell_str[j,1]] * sigma_m[1] + 
      ...
      alpha_age_eth[(cell_str[j,2]-1) * J_age + cell_str[j,1]] * lambda_inter * sigma_m[1] * sigma_m[2];
    //+
...
generated quantities{
  ...
  pri_var <- pow(sigma_m[1],2) + 
    pow(sigma_m[2],2) + 
    //pow(sigma_m[3],2) + 
    pow(lambda_inter * sigma_m[1] * sigma_m[2],2);
    //+
