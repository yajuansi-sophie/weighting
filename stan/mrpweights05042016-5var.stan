...
for (j in 1:J_true){
  mu_cell_pred[j] <- alpha + alpha_age[cell_str[j,1]] * sigma_m * lambda_m[1] +  alpha_eth[cell_str[j,2]] * sigma_m * lambda_m[2] + 
  alpha_edu[cell_str[j,3]] * sigma_m * lambda_m[3] +
  alpha_sex[cell_str[j,4]] * sigma_m * lambda_m[4] + 
  alpha_inc[cell_str[j,5]] * sigma_m * lambda_m[5] +
  alpha_eld[cell_str[j,6]] * sigma_m * lambda_m[6] + 
  alpha_cld[cell_str[j,7]] * sigma_m * lambda_m[7] +
  alpha_psx[cell_str[j,8]] * sigma_m * lambda_m[8] + 
  ...
  alpha_eth_inc[(cell_str[j,5]-1) * J_eth + cell_str[j,2]] * lambda_inter[1] * lambda_m[2] * lambda_m[5] * sigma_m +
  alpha_age_inc[(cell_str[j,5]-1) * J_age + cell_str[j,1]] * lambda_inter[1] * lambda_m[1] * lambda_m[5] * sigma_m +
  alpha_inc_psx[(cell_str[j,8]-1) * J_inc + cell_str[j,5]] * lambda_inter[1] * lambda_m[5] * lambda_m[8] * sigma_m + 
  alpha_inc_eldx[(cell_str[j,6]-1) * J_inc + cell_str[j,5]] * lambda_inter[1] * lambda_m[5] * lambda_m[6] * sigma_m + 
  alpha_inc_cldx[(cell_str[j,7]-1) * J_inc + cell_str[j,5]] * lambda_inter[1] * lambda_m[5] * lambda_m[7] * sigma_m + 
  ...
  alpha_age_eth_inc[(cell_str[j,5]-1) * J_eth * J_age + (cell_str[j,2]-1) * J_age + cell_str[j,1]] * lambda_inter[2] * lambda_m[1] * lambda_m[2] * lambda_m[5] * sigma_m;
  }
